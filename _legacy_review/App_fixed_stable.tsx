...
import { AppData, Bank, ClientMessage, Company, Currency, ServerMessage, Transfer } from '../src/types';
...
const loadAllFromSupabase = async () => {
  ...
  const mappedTransfers: Transfer[] = (transfersData || [])
    .filter((t: any) => t && t.id && t.company_id && t.bank_id)
    .map((t: any) => {
      const safeDate = t.transfer_date || t.date || format(new Date(), 'yyyy-MM-dd');
      const safeTimestamp = t.created_at || t.timestamp || `${safeDate}T00:00:00.000Z`;
      ...
      return {
        ...
        note: t.note && t.note !== 'EMPTY' ? String(t.note) : '',
        currency: t.currency === 'CNY' ? 'CNY' : 'USD',
        date: safeDate,
...
const sendMessage = async (msg: ClientMessage) => {
  ...
  if (msg.type === 'ADD_TRANSFER') {
    ...
    const safeDate = savedTransfer.transfer_date || savedTransfer.date || msg.date;
    const safeTimestamp = savedTransfer.created_at || `${safeDate}T00:00:00.000Z`;
    ...
    setData((prev) => ({
      ...
      transfers: [
        ...prev.transfers,
        {
          ...
          companyId: String(savedTransfer.company_id),
          bankId: String(savedTransfer.bank_id),
          amount: Number(savedTransfer.amount || 0),
          note: savedTransfer.note && savedTransfer.note !== 'EMPTY' ? String(savedTransfer.note) : '',
          currency: savedTransfer.currency === 'CNY' ? 'CNY' : 'USD',
          date: safeDate,
          timestamp: safeTimestamp,
        },
      ],
  ...
  if (msg.type === 'UPDATE_TRANSFER') {
    ...
    const safeDate = updatedTransfer.transfer_date || updatedTransfer.date || format(new Date(), 'yyyy-MM-dd');
    const safeTimestamp = updatedTransfer.created_at || updatedTransfer.timestamp || `${safeDate}T00:00:00.000Z`;
      ...
      transfers: prev.transfers.map((transfer) =>
        transfer.id === msg.id
          ? {
              ...
              companyId: String(updatedTransfer.company_id),
              bankId: String(updatedTransfer.bank_id),
  ...
  if (msg.type === 'DELETE_TRANSFER') {
    const confirmDelete = window.confirm('Ин гузариш нест карда шавад?');
    if (!confirmDelete) return;

    const { error } = await supabase.from('transfers').delete().eq('id', msg.id);
    if (error) return alert('خطا در حذف انتقال: ' + error.message);

    setData((prev) => ({
      ...prev,
      transfers: prev.transfers.filter((transfer) => transfer.id !== msg.id),
    }));
...
function CompanyCard({
  ...
  const saveEdit = () => {
    ...
    if (isNaN(value) || value <= 0) {
      alert('Маблағ бояд аз 0 калон бошад');
      return;
    }
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onBlur={() => {
                ...
                const parsedReturn = parseFloat(returnInput) || 0;
                if (parsedReturn === returnedAmount) return;
                onUpdateReturn(parsedReturn);
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
